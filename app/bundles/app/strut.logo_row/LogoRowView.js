	...
	function(Backbone, LogoRowModel)
	{
		...
		return Backbone.View.extend(
		{
			...
			render: function()
			{
				...
				if (isElectron())
				{
					// Add the Tag Library Menu Item (The next 4 items)
					var $library_dropdown = this.$el.find('.dropdown-menu[data-menu_item="library"]');
					for (var i = 13; i < 17; i++)
					{
						var item = this.model.items[i];
						$library_dropdown.append(item.render().$el);
					}

					// Add the Electron Confiuration Menu Item
					var $config_menu_item = this.$el.find('li[data-menu_item="configuration"]');
					var item = this.model.items[17];
